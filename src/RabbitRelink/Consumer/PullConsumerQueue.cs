...
{
    internal class PullConsumerQueue : IDisposable
    {
        #region Fields
        ...
        public Task<Acknowledge> PutAsync(ConsumedMessage<byte[]> message)
        {
            var completion = new TaskCompletionSource<Acknowledge>();
            var msg = new PulledMessage<byte[]>(message, completion);

            if (msg.Cancellation.IsCancellationRequested || _disposedCancellation.IsCancellationRequested)
                return Task.FromCanceled<Acknowledge>(msg.Cancellation);
        ...
        public async Task<PulledMessage<byte[]>> TakeAsync(CancellationToken cancellation)
        {
            while (true)
            {
        ...
        private class QueueItem
        {
            ...
            private readonly TaskCompletionSource<Acknowledge> _completion;

            #endregion

            #region Ctor

            public QueueItem(PulledMessage<byte[]> message,
            ...
            public PulledMessage<byte[]> Message { get; }
